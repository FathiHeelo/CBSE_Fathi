...
export function MealCard({ meal, onAdd }: MealCardProps) {
  return (
    <Card sx={{ display: 'flex', flexDirection: { xs: 'column', sm: 'row' }, height: '100%', overflow: 'hidden' }}>
      <CardMedia alt={meal.name} component="img" image={meal.image} loading="lazy" sx={{ aspectRatio: { xs: '16 / 9', sm: '1 / 1' }, flex: { sm: '0 0 160px' }, objectFit: 'cover', width: { xs: '100%', sm: 160 } }} />
      <CardContent sx={{ display: 'flex', flex: 1, flexDirection: 'column', p: 2 }}>
        ...
        <Stack alignItems="center" direction="row" justifyContent="space-between" spacing={2} sx={{ mt: 'auto', pt: 2 }}>
          ...
          <Button color="secondary" startIcon={<AddShoppingCartRounded />} variant="contained" onClick={() => onAdd(meal)}>
            Add to cart
          </Button>
        </Stack>
      </CardContent>
